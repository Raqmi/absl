    ...
    class OrderScreen extends IndependentToOrderScreen {
        ...
        get filteredOrders() {
            const filterCheck = (order) => {
                if (this.filter && this.filter !== 'Todos los pedidos') {
        ...
        get _searchFields() {
            var fields = {
                'Ticket/Ref': (order) => order.name,
                'Recibo/Ref': (order) => order.pos_reference,
                'Fecha (YYYY-MM-DD)': (order) => moment(order.creation_date).format('YYYY-MM-DD hh:mm A'),
                'Vendedora': (order) => order.salesman_id[1],
                'Estado del pedido ': (order) => order.state,
        ...
        get_journal_from_order(statement_ids) {
            ...
            PaymentPromise.then(function(statements){
                ...
                if(order_statements){
                    ...
                    console.log("ABARROTES SAN LUIS: ALERTA CONEXION PERDIDA !!!!");
                }
             })
        }
