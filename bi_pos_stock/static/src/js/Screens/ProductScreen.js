					...
					if(self.env.pos.config.show_stock_location == 'specific' && product.type == 'product')
					{
						...
						await this.rpc({
							...
							if (allow_order == false)
							{
								if ( (output[0][1] <= deny_order) || (output[0][1] <= 0) )
								{
									...
									self.showPopup('ErrorPopup', {
										title: self.env._t('Abarrotes San Luis'),
										body: self.env._t("Alerta " + "(" + product.display_name + ")" + " No tienes existencia."),
									});
								}
							}
							else if(allow_order == true)
							{
								if (output[0][1] <= deny_order)
								{
									...
									self.showPopup('ErrorPopup', {
										title: self.env._t('Abarrotes San Luis'),
										body: self.env._t("Alerta " + "(" + product.display_name + ")" + " Agotado."),
									});
								}
							}
						});
						...
						if (product.type == 'product' && allow_order == false)
						{
							if (product.qty_available <= deny_order && allow_order == false)
							{
								...
								self.showPopup('ErrorPopup', {
									title: self.env._t('Abarrotes San Luis'),
									body: self.env._t("Alerta " + "(" + product.display_name + ")" + " Producto agotado."),
								});
							}
							else if (product.qty_available <= 0 && allow_order == false)
							{
								...
								self.showPopup('ErrorPopup', {
									title: self.env._t('Abarrotes San Luis'),
									body: self.env._t("(" + product.display_name + ")" + " Agotado."),
								});
							}
						}
							...
							call_super = false; 
							self.showPopup('ErrorPopup', {
								title: self.env._t('Error: sin stock'),
								body: self.env._t("(" + product.display_name + ")" + " Agotado."),
							});
						}
					}
					...
					if (pos_config.show_stock_location == 'specific')
					{
						...
						await this.rpc({
							...
							if(flag > 0){
								...
								self.showPopup('ErrorPopup', {
									title: self.env._t('Abarrotes San Luis'),
									body: self.env._t('Uno o mas de los productos seleccionados no tienen existencia.'),
								});
							}
						});
					} else {
						$.each(lines, function( i, line ){
							if (line.product.type == 'product'){
								if (allow_order == false && line.quantity > line.product['bi_on_hand']){
									...
									self.showPopup('ErrorPopup', {
										title: self.env._t('Abarrotes San Luis'),
										body: self.env._t('Uno o mas de los productos no tienen existencia.'),
									});
									return
								}
								...
								if(allow_order == true && check < deny_order){
									...
									self.showPopup('ErrorPopup', {
										title: self.env._t('Abarrotes San Luis'),
										body: self.env._t('Uno o mas de estos productos no tienen existencia.'),
									});
									return
								}
							}
