...
class wizard_xz_report(models.TransientModel):
    ...
    session_ids = fields.Many2many(comodel_name='pos.session', string="Sesion")
    start_date = fields.Datetime(string="Fecha Inicial")
    end_date = fields.Datetime(string="Fecha Final")
    show_top_saling_product = fields.Boolean(string="Mostrar productos mas vendidos")
    no_of_product_disp = fields.Integer(string="Numero de productos a mostrar", default=5)
    display_category_product = fields.Boolean(string="Quieres ver los productos por categoria?")

    def get_data(self):
        data = self.read()[0]
        domain = []
        if all([not data.get(fld) for fld in ['start_date', 'end_date', 'session_ids']]):
            raise ValidationError(_("Seleccione al menos un criterio para imprimir el informe."))
        if self.session_ids:
        ...
        if not session_ids:
            raise ValidationError(_("No hay una sesion con estos criterios."))
