        ...
        class extends ProductScreen {
            ...
            async _onClickPay() {
                if(this.env.pos.user.pos_user_type === "salesman" && this.env.pos.config.enable_order_sync){
                    ...
                    var order_str =  currentOrder.get_is_modified_order() ? " Modify " : " Pedido en Caja ";
                    const { confirmed } = await this.showPopup('CreateDraftOrderPopup', {
                        title: this.env._t('Abarrotes San Luis Mayoreo'),
                        body: this.env._t('Quieres enviar el pedido a Caja?'),
                    });
                    if (confirmed){
                            ...
                            if(managers.length > 0){
                                ...
                                if(confirmed){
                                    ...
                                    if(result_find){
                                        ...
                                        alert(_t('Por favor ingresa PIN/Barcode correcto!'));
                                        return;
                                    }
                                }
                            }else{
                                alert(_t('Pide ayuda con el gerente de mayoreo!'));
                                return;
                            }
                        }
                    }
